...
class StatsController {
    ...
    private getContentItems(
        ...
        const addItems = (items: TopologyNodeDataStatsItem[]) => {
            items.forEach(({name, value}) => {
                ...
                nextTop +=
                    Math.max(stat.getScaledHeight(), val.getScaledHeight()) + NodeSize.textOffset;
            });
        };
    ...
    private createGroup(titles: fabric.Text[], line: fabric.Path, content: StatsContent[]) {
    ...
    private initListeners() {
        this.content.forEach(({group, title, items, hoverLine}) => {
            title.on('mousedown', () => {
                ...
                const currentContent = this.content.find((item) => item.group === currentGroup);
...
export function getStats(
    ...
    const sC = new StatsController(canvas, stats, {top: statsTop, left: statsLeft}, colors);
