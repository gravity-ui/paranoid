...
import {AnchorSizes, CLIPBOARD_WIDTH, NodeSizes} from './constants';
...
function calculateHeight(
    ...
    if (
        ...
        return (
            ...
            (metrics ? NodeSizes.metricsMarginTop + metrics.getBoundingRect().height : 0) +
...
export default function renderNode(
    ...
    const meta = new fabric.Text(node.meta || '', {
        ...
        left: NodeSizes.paddingLeft,
        top: NodeSizes.paddingTop + title.getBoundingRect().height + NodeSizes.metaMarginTop,
        fontFamily: NODE_FONT_FAMILY,
    ...
    if (textOverflow === TextOverflow.Ellipsis) {
        ...
        const statusGroupWidth = statusGroup ? statusGroup.getBoundingRect().width : 0;
        const metricsWidth = metrics
            ? metrics.getBoundingRect().width + NodeSizes.paddingLeft + NodeSizes.paddingRight
            : 0;
    ...
    const nodeGroup = [bodyRect, title, clipboard, meta, statusGroup, metrics].filter(
        Boolean,
    ) as fabric.Object[];
