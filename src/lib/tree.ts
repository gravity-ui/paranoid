...
export function traverseDF(root: TreeNode, fn: (node: TreeNode, leaf: boolean) => void) {
    const children = [root];
...
export function traverseLeftBranch(root: TreeNode, fn: (node: TreeNode, leaf: boolean) => void) {
    const children = [root];
    while (children.length) {
        const node = children.shift();
        if (node) {
