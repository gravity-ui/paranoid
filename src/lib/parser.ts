...
export default class Parser {
    ...
    private getAppender(data: ElementWithChildren[], groups: Record<string, TreeNode>) {
        ...
        const appendNodesToTree = (root: TreeNode, children: string[]) => {
            const treeNodeChildren = children.map((child) => {
                const node = data.find(({name}) => name === child) as ElementWithChildren;
            ...
            if (parentGroupName && groupChildren.length > 0) {
                const parentGroupChildren = notGroupMemebersChildren.get(parentGroupName);
