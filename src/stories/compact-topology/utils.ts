...
export function prepareHosts(hosts: PgHost[]) {
    ...
    let group: string;
    const hostsWithoutReplicas = hosts.filter(({replicationSource}) => !replicationSource);
...
export function prepareExplainData(tables: any) {
    ...
    _.forEach(tables, (table) => {
        ...
        _.forEach([...reads, ...writes], (node) => {
            ...
            const nodeId = getExplainNodeId(table.name, node.type, String(tableTypes[node.type]));
